...
export async function main(args: string[]) {
	...
	// agent-beat: copy bundled fd/rg into $getAgentDir()/bin so the first Grep/Find
	// call resolves them locally instead of hitting ensureTool()'s GitHub download.
	// Must run before the best-of-N fork so children reuse the seeded copies.
	try {
		const { seedBundledBinaries } = await import("./utils/tools-manager.js");
		seedBundledBinaries();
	} catch {
		// non-fatal
	}
	...
	// agent-beat: best-of-N. When running as a solver and not already a child
	// worker (TAU_BON=0), fork N independent copies against git clones of the
	// original repo, pick the largest non-empty patch, apply it back, and
	// relay the winner's stdout to the validator. Children recursively enter
	// main() with TAU_BON=0 and run single-shot via runPrintMode below.
	if (solverLikeRun && process.env.TAU_BON !== "0") {
		const n = Math.max(1, Number.parseInt(process.env.TAU_BON ?? "3", 10) || 3);
		if (n > 1) {
			takeOverStdout();
			const { runBestOfN } = await import("./core/solver-bon.js");
