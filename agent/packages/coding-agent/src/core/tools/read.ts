...
import { formatHashLines } from "./line-hash.js";
...
export function createReadToolDefinition(
        ...
        return {
                name: "read",
                label: "read",
                description: `Read the contents of a file. Each text line is prefixed with LINE#HASH: anchors. Use these anchors with the edit tool to make precise, hash-anchored edits. Supports text files and images (jpg, png, gif, webp). For text files, output is truncated to ${DEFAULT_MAX_LINES} lines or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first). Use offset/limit for large files.`,
                promptSnippet: "Read file contents with hashline anchors (LINE#HASH: prefix)",
                promptGuidelines: [
                        "Use read to examine files. Output includes LINE#HASH: prefixes for use with the edit tool.",
                        "When you need to edit, use the LINE#HASH anchors from read output to specify exact edit locations.",
                ],
                                        ...
                                        (async () => {
                                                try {
                                                        ...
                                                        if (mimeType) {
                                                                ...
                                                                // Add hashline prefixes for hash-anchored editing.
                                                                outputText = formatHashLines(outputText, startLineDisplay);
